                ...
                <div class="form-create-item-content">
                    <div class="form-create-item">
                        <div class="sc-heading">
                            <h3>Update Item</h3>
                            <p class="desc">Update Your Porduct Detail Here to get sell it faster </p>
                        </div>
                        ...
                        @endif
                        <form id="create-item-1" action="{{ url('/edit-product/' . $product->id) }}" method="post" accept-charset="utf-8" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            <div class="upload-container">
                                ...
                                <input type="file" class="inputfile form-control" name="pro_image[]" multiple style="display: none;">
                                <div class="image-preview">
                                @foreach ($product->images as $image)
                                        <div class="image-container">
                                            <img src="{{ asset('storage/' . $image->product_image) }}" class="thumbnail">
                            ...
                            <p id="image-limit-msg" style="color: red; font-size: 14px;">You can upload up to 15 images.</p>


                            <div class="input-group">
                                <input name="title" value="{{ old('title', $product->title) }}" type="text" placeholder="Item Title"  maxlength="70" required>
                            </div>
                            <textarea id="comment-message" name="description" tabindex="4" placeholder="Description" aria-required="true" required>{{ old('description', $product->description) }}</textarea>
                            <div class="input-group" style="margin-bottom: 10px">
                                <select name="cat_id" id="category" placeholder="category" style="margin-right: 10px" required>
                                ...
                                <select name="condition" id="condition" placeholder="Condition" required>
                                    ...
                                    <option value="box packed" {{ $product->condition == 'box packed' ? 'selected' : '' }}>Box Packed</option>
                                    <option value="new" {{ $product->condition == 'new' ? 'selected' : '' }}>New</option>
                                    <option value="used" {{ $product->condition == 'used' ? 'selected' : '' }}>Used
                                    </option>
                                    <option value="10/9" {{ $product->condition == '10/9' ? 'selected' : '' }}>10/9
                                    </option>
                                    <option value="10/8" {{ $product->condition == '10/8' ? 'selected' : '' }}>10/8
                                    </option>
                                    <option value="low" {{ $product->condition == 'low' ? 'selected' : '' }}>Low</option>
                                </select>
                            </div>
                            <input id="itemPrice" name="price" value="{{ old('price', $product->price) }}" type="text" placeholder="Item Price" oninput="formatPrice(this)" style="margin-bottom: 10px" required>

                            <div class="input-group">
                                <input name="city" value="{{ old('city', $product->city) }}" type="text" placeholder="City" required>
                                <input name="area" value="{{ old('area', $product->area) }}" type="text" placeholder="Area" required>
                            </div>
                            <div class="input-group">
                                <div class="keyword-container">
                                    <div class="keywords-input">
                                        <input type="text" id="keyword-input" placeholder="Add up to 5 keywords" maxlength="30">
                                        <ul id="keywords-list">
                                        @foreach ($product->keywords as $keyword)
                                            <li>{{ $keyword->keywords }} <span class="remove-keyword">x</span></li>
                                        @endforeach
                                        </ul>
                            ...
                            <div class="input-group style-2 ">
                                <div class="btn-check">
                                    <input type="radio" id="sale" name="type" value="0" required onchange="togglePriceField()>
                                    <label for="sale">Fixed Price</label>
                                </div>
                                <div class="btn-check">
                                    <input type="radio" id="price" name="type" value="1" required onchange="togglePriceField()>
                                    <label for="price">
                                        Auction
                                    </label>
                                </div>
                            ...
                            <button type="submit" class="sc-button style letter style-2"><span>Update Item</span></button>
                         </form>
                    </div>
